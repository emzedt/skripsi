<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto p-4 mt-8">
        ...
        <input type="hidden" name="foto_masuk" id="photo-data">
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            async function loadModels() {
                try {
                    // Show loading while models are loading
            ...
            async function submitAttendance() {
                ...
                try {
                    showLoading('Mengirim data absensi...');

                    // Verifikasi bahwa detectedUser sama dengan user yang login
                    const loggedInUserId = "{{ Auth::id() }}"; // ID user yang login
                    const detectedUserId = detectedUser.split('_')[1]; // Format label: nama_id

                    if (detectedUserId !== loggedInUserId) {
                        throw new Error('Wajah tidak sesuai dengan akun yang login');
                    }

                    // Create FormData for better handling
                    const formData = new FormData();
                    formData.append('username', detectedUser);
                    formData.append('foto_masuk', capturedImage);
                    formData.append('latitude', latitudeInput.value);
                    formData.append('longitude', longitudeInput.value);
                    formData.append('_token', document.querySelector('meta[name="csrf-token"]').content);

                    const response = await fetch("{{ route('absensi-masuk.store') }}", {
                        method: 'POST',
                        body: formData,
                        headers: {
                            'Accept': 'application/json',
                    ...
                    let message = result.message || 'Absensi berhasil';
                    let icon = 'success';
