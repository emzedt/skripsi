            ...
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex justify-end mb-4">
                        @hasPermission('Tambah Pengajuan Cuti')
                            <a href="{{ route('pengajuan_cuti.create') }}"
                    ...
                    <table id="cuti-table" class="min-w-full dt-tailwindcss divide-y divide-gray-200"
                        style="width: 100%">
        ...
        <script>
            function confirmDelete(id, name) {
                Swal.fire({
                    ...
                    text: `Pengajuan cuti "${name}" akan dihapus!`,
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        fetch(`/pengajuan-cuti/${id}`, { // Sesuaikan URL jika perlu
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}', // Cara standar
                                'Content-Type': 'application/json'
                            }
                        }).then(response => {
                            if (response.ok) {
                                $('#cuti-table').DataTable().ajax.reload(null, false);
                                Swal.fire({
                                    title: 'Berhasil!',
            ...
            $(document).ready(function() {
                $('#cuti-table').DataTable({
                    ...
                    autoWidth: false,
                    scrollX: true,
                    ajax: "{{ route('pengajuan_cuti.index') }}", // <-- Koma dihapus dari sini
                    language: {
                        lengthMenu: '_MENU_',
                        search: '',
