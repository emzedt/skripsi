            ...
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div class="flex justify-end mb-4">
                        @hasPermission('Tambah Permintaan Lembur')
                            <a href="{{ route('permintaan_lembur.create') }}"
                    ...
                    <table id="lembur-table" class="min-w-full dt-tailwindcss divide-y divide-gray-200">
        ...
        <script>
            function confirmDelete(id, nama, tanggal) {
                Swal.fire({
                    ...
                    html: `Permintaan lembur oleh <strong>${nama}</strong> pada tanggal <strong>${tanggal}</strong> akan dihapus.`,
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        fetch(`/permintaan-lembur/${id}`, { // Sesuaikan URL dengan route Anda
                            method: 'DELETE',
                            headers: {
                            ...
                            if (response.ok) {
                                window.lemburTable.ajax.reload(null, false);
                                Swal.fire('Berhasil!', 'Permintaan lembur telah dihapus.', 'success');
                            } else {
            ...
            $(document).ready(function() {
                window.lemburTable = $('#lembur-table').DataTable({
                    processing: true,
                    serverSide: true,
                    responsive: true,
                    ajax: "{{ route('permintaan_lembur.index') }}",
                    language: {
                        lengthMenu: '_MENU_',
                        search: '',
